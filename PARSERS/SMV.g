...
//primary_expr_select
//			:
//			( TOK_LB f=simple_expression
//				( TOK_RB 
//					-> ^(ARRAY_INDEX_T $f)
//				| TOK_COLON s=simple_expression TOK_RB 
//					-> ^(BIT_SELECT_T $f $s)
//				)
//			| TOK_DOT
//				( TOK_ATOM
...
constru_for_expr
			:  (TOK_AND TOK_FOR TOK_LP
					i_and=TOK_ATOM TOK_EQUAL init_and=simple_expression TOK_SEMI
					cond_and=simple_expression TOK_SEMI
					ii_and=TOK_ATOM TOK_EQUAL inc_and=simple_expression
				TOK_RP
				TOK_LCB
					loop_main=simple_expression
				TOK_RCB)
					-> ^(TOK_AND_FOR_T TOK_LP 
							$i_and NOP
							$init_and NOP
							$cond_and NOP
							$inc_and NOP
						...
						TOK_LCB
							$loop_main
						TOK_RCB)
			| (TOK_OR TOK_FOR TOK_LP
				i_or=TOK_ATOM TOK_EQUAL init_or=simple_expression TOK_SEMI
				cond_or=simple_expression TOK_SEMI
				ii_or=TOK_ATOM TOK_EQUAL inc_or=simple_expression
				TOK_RP
				TOK_LCB
					loop_main=simple_expression
				TOK_RCB)
					-> ^(TOK_OR_FOR_T TOK_LP 
						...
						TOK_LCB
							$loop_main
						TOK_RCB)
			| (TOK_PLUS TOK_FOR TOK_LP
				i_plus=TOK_ATOM TOK_EQUAL init_plus=simple_expression TOK_SEMI
				cond_plus=simple_expression TOK_SEMI
				ii_plus=TOK_ATOM TOK_EQUAL inc_plus=simple_expression
				TOK_RP
				TOK_LCB
					loop_main=simple_expression
				TOK_RCB)
					-> ^(TOK_PLUS_FOR_T TOK_LP 
							$i_plus NOP
							$init_plus NOP
							$cond_plus NOP
							$inc_plus NOP
...
case_element_list_expr
			: case_element_expr (case_element_expr)*
			;
case_element_expr
			...
			;
concatination_expr
			: primary_expr (TOK_CONCATENATION^ primary_expr)*
			;
multiplicative_expr
			: concatination_expr 
...
/* right association */
implies_expr
			: iff_expr (TOK_IMPLIES^ implies_expr)?
			;
...
//			;
//type		: ((TOK_ARRAY subrange TOK_OF)* TOK_ATOM) => module_type
//				-> ^(SYNC_PROCESS_T module_type)
//			| itype
//			| TOK_PROCESS module_type
//				-> ^(ASYNC_PROCESS_T module_type)
...
module_sign	: TOK_ATOM
			...
			| TOK_ATOM TOK_LP TOK_RP
				...
				-> ^(MODULE_SIGN_T TOK_ATOM)
			| TOK_ATOM { curr_module_sign_vector.add($TOK_ATOM.text); //the module name
			  	     curr_module_sign_vector.add("0"); //"1" -- is Agent; "0" -- is normal module
...
declarations
			: (declaration)*;
sign_atom_list
			:  f=TOK_ATOM { curr_module_sign_vector.add($f.text); //the argument name
		  			curr_module_sign_vector.add("0"); //"1" -- visible; "0" -- invisible
		  		      }
				(TOK_COMMA! r=TOK_ATOM { curr_module_sign_vector.add($r.text); //the argument name
		  			curr_module_sign_vector.add("0"); //"1" -- visible; "0" -- invisible
		  		})*
			;
...
//		  -> ^(AGENT_DECL_INVISIBLE_VAR_T agent_invisible_var)
		...
		| avar // action variable declaration
		| prot // protocol declaration
		| assign 
		| init
		| trans
		| define
...
agent_invisible_input_var	: TOK_INVISIBLE_IVAR
				| TOK_INVISIBLE_IVAR^ ivar_decl_list
				;

// avar is for the declaration of action variable
avar :  TOK_AVAR TOK_COLON avar_type TOK_SEMI 
...
// the protocol
prot	: TOK_PROT_BEGIN prot_element_list_expr TOK_PROT_END ;

prot_element_list_expr	: prot_element_expr (prot_element_expr)* 
		;
		
prot_element_expr	: basic_expr TOK_COLON basic_expr TOK_SEMI
		-> ^(PROT_ELEMENT_EXPR_T basic_expr basic_expr)
		;
...
var_decl	: decl_var_id TOK_COLON type TOK_SEMI
			...
			| (TOK_FOR TOK_LP
					i_f=TOK_ATOM TOK_EQUAL init_f=simple_expression TOK_SEMI
					cond_f=simple_expression TOK_SEMI
					ii=TOK_ATOM TOK_EQUAL inc_f=simple_expression
				TOK_RP
				TOK_LCB loop_main=var_decl_list TOK_RCB)
					-> ^(TOK_FOR TOK_LP $i_f NOP $init_f NOP $cond_f NOP $inc_f NOP TOK_RP
							TOK_LCB $loop_main TOK_RCB)
			;
...
ivar_decl	: decl_var_id TOK_COLON type TOK_SEMI
			...
			| (TOK_FOR TOK_LP
					...
					ii=TOK_ATOM TOK_EQUAL inc_f=simple_expression
				TOK_RP
				TOK_LCB loop_main=ivar_decl_list TOK_RCB)
					-> ^(TOK_FOR TOK_LP $i_f NOP $init_f NOP $cond_f NOP $inc_f NOP TOK_RP
							TOK_LCB $loop_main TOK_RCB)
			;
...
define_list	: define_decl (define_decl)*
			;
define_decl	: decl_var_id TOK_EQDEF simple_expression TOK_SEMI
				-> ^(DEFINE_DECL_T decl_var_id simple_expression)
			| (TOK_FOR TOK_LP
					i_f=TOK_ATOM TOK_EQUAL init_f=simple_expression TOK_SEMI
					cond_f=simple_expression TOK_SEMI
					ii=TOK_ATOM TOK_EQUAL inc_f=simple_expression
				TOK_RP
				TOK_LCB loop_main=define_list TOK_RCB)
					-> ^(TOK_FOR TOK_LP $i_f NOP $init_f NOP $cond_f NOP $inc_f NOP TOK_RP
							TOK_LCB $loop_main TOK_RCB)
			;
...
one_assign	: var_id TOK_EQDEF simple_expression TOK_SEMI
			...
			| (TOK_FOR TOK_LP
					...
					ii=TOK_ATOM TOK_EQUAL inc_f=simple_expression
				TOK_RP
				TOK_LCB loop_main=assign_list TOK_RCB)
					-> ^(TOK_FOR TOK_LP $i_f NOP $init_f NOP $cond_f NOP $inc_f NOP TOK_RP
							TOK_LCB $loop_main TOK_RCB)
			;
...
justice		: TOK_JUSTICE^ justice_list
			;
justice_list
			: justice_decl (justice_decl)*
			;
justice_decl
			: simple_expression TOK_SEMI
			...
			| (TOK_FOR TOK_LP
					...
					ii=TOK_ATOM TOK_EQUAL inc_f=simple_expression
				TOK_RP
				TOK_LCB loop_main=justice_list TOK_RCB)
					-> ^(TOK_FOR TOK_LP $i_f NOP $init_f NOP $cond_f NOP $inc_f NOP TOK_RP
							TOK_LCB $loop_main TOK_RCB)
			;
compassion	: TOK_COMPASSION^ compassion_list
			;
compassion_list
			: compassion_decl (compassion_decl)*
			;
compassion_decl
			: TOK_LP f=simple_expression TOK_COMMA s=simple_expression TOK_RP TOK_SEMI
			...
			| (TOK_FOR TOK_LP
					...
					ii=TOK_ATOM TOK_EQUAL inc_f=simple_expression
				TOK_RP
				TOK_LCB loop_main=compassion_list TOK_RCB)
					-> ^(TOK_FOR TOK_LP $i_f NOP $init_f NOP $cond_f NOP $inc_f NOP TOK_RP
							TOK_LCB $loop_main TOK_RCB)
			;
/* not implemented in JTLV */
...
avoidtarget	: TOK_AVOIDTARGET player_num simple_expression optsemi
			...
			;
reachdeadlock
			: TOK_REACHDEADLOCK player_num optsemi
				-> ^(TOK_REACHDEADLOCK player_num)
			;
avoiddeadlock
			: TOK_AVOIDDEADLOCK player_num optsemi
...
buchigame	: TOK_BUCHIGAME player_num TOK_LP simple_list_expression TOK_RP optsemi
			...
			;
genreactivity
			: TOK_GENREACTIVITY player_num TOK_LP simple_list_expression TOK_RP TOK_IMPLIES TOK_LP simple_list_expression TOK_RP optsemi
...
TOK_INVISIBLE_IVAR	: 'INVISIBLEIVAR';
TOK_AVAR		: 'ACTION';//                  {yylval.lineno = yylineno; return(TOK_AVAR);}
TOK_PROT_BEGIN		: 'PROTOCOL';
TOK_PROT_END		: 'END';
//TOK_PROT_ELSE		: 'ELSE';
