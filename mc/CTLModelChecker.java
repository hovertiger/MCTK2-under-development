...
public class CTLModelChecker extends ModelChecker {
	...
	public void modelCheck(Spec property) throws ModelCheckException,
		...
		if (!(property.isCTLSpec()|property.isRealTimeCTLSpec()))
	...
	private BDD CTLAux(Spec property) throws ModelCheckException {
        ...
        //Uppdate by LS on : 2017/10/20
        //System.out.println("numOfOperand:  "+op.numOfOperands());
        //System.out.println("child0:  "+child[0]);
        //System.out.println("child1:  "+child[1]);
        //System.out.println("child2:  "+child[2]);
        ...
        if (noo==2) {//ABF, ABG, EBF, EBG  left or right
            ...
            { range = (SpecRange) child[0];
              left= CTLAux(child[1]);}//xxxxxxxx
            else
            {left=CTLAux(child[0]);//AU GU
             right=CTLAux(child[1]);
            }
        }
        if (noo==3)// ABU, EBU
        {
            ...
            { range = (SpecRange) child[1];
                left=CTLAux(child[0]);//xxxxxxxx
                right=CTLAux(child[2]);//xxxxxxxxx
            }
        }
	...
	//Uppdate by LS on : 2017/10/23
	// E[p BU from..to q] under fairness  测试通过
	public BDD EfBU(int from, int to, BDD p, BDD q) {//**************
//        if (this.ctlFair == null)
//            ctlFair = ce_fair_g(Env.TRUE());
//        return allPredsIn(p, q.id().and(ctlFair));//返回不动点
//		System.out.println("测试EfBU！！！");
		if(from==0 & to==0)
			return q.id();
		if(from>0)//(Operator.AND, new Spec[]{leftBaseSpec, new SpecExp(Operator.NEXT,transBUToLTLSPec(leftBaseSpec, baseSpec, a - 1, b))});
				return p.id().and(EfX(EfBU(from-1, to-1, p, q)));
		if(to>0)
				return q.id().or(p.id().and(EfX(EfBU(from, to-1, p, q))));
		return null;
	}
...
//		return z;//返回不动点
		if(from==0 & to==0)
			return p.id(); //return baseSpec;
		if(from>0)
			return EfX(EfBG(from-1, to, p));//SpecExp(Operator.NEXT, transBGToLTLSPec(baseSpec, a - 1, b));
		//SpecExp(Operator.AND, new Spec[]{leftBaseSpec, new SpecExp(Operator.NEXT,transBUToLTLSPec(leftBaseSpec, baseSpec, a - 1, b))});
		if(to>0)//SpecExp(Operator.AND, new Spec[]{baseSpec,new SpecExp(Operator.NEXT,transBGToLTLSPec(baseSpec, a, b - 1))});
			return p.id().and(EfX(EfBG(from, to-1, p)));
		return null;
	}

	// A[p BU from..to q] under fairness
	public BDD AfBU(int from, int to, BDD p, BDD q) {//rrrrrrrrrrrr
        return  EfBF(0,from-1,p.id().not()).or(EfBG(from,from,EfBG(0,to-from,q.id().not()))).
                 or(EfBU(0,to-from,q.id().not(),q.id().not().and(p
                .id().not())));//EfBU(from,to,q.id().not(), p.id().not().and(q.id().not())).not().and(EfG(q.id().not()).not());
	}
	...
	public BDD allPredsIn(BDD p, BDD q) {
		...
		for (FixPoint<BDD> ires = new FixPoint<BDD>(); ires.advance(q);)
			q = q.id().or(p.and(design.pred(q.id())));//q \/ p & next(q)  符号化模型检测49页
		return q;
	}
