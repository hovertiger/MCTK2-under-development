...
public class ModelCheckingTest {
	public static void main(String[] args) throws ModelCheckException {
/*
		simpleCheckDeadlock();
		Env.resetEnv();
...
//		Check_test();
	...
	public static void ctlCheck() {
		...
		//Spec[] all_specs = Env.loadSpecString("SPEC (agent1 KNOW TRUE) ");
		Spec[] all_specs = Env.loadSpecString(to_parse);
		System.out.println("========= DONE Loading Specs ============");
...
*/		try {
	...
	public static void rtctlCheck() {
		...
		String to_parse = "";
		//to_parse += "CTLSPEC  EF rangeValue=1 & EF a=1 ;\n"; // 0
		//to_parse += "CTLSPEC E [a=1 BU 6..10 a=2 ];\n"; // 0 T
		//to_parse += "CTLSPEC EBF 6..10 a=1;\n"; // 0 F
		//to_parse += "CTLSPEC EBF 6..10 a=2;\n"; // 0 T
		//to_parse += "CTLSPEC EBF 6..10 a=3;\n"; // 0 F

		     to_parse += "CTLSPEC A [a=1 BU 6..12 a=0 ];\n"; // 1
		     to_parse += "CTLSPEC A [a=1 BU 6..12 a=2 ];\n"; // 1
		//to_parse += "CTLSPEC EBF 6..10 a=2;\n"; // 2
		     to_parse += "CTLSPEC ABF 6..10 a=2;\n"; // 3
		     to_parse += "CTLSPEC EBG 6..10 a=2;\n"; // 4
		//to_parse += "CTLSPEC ABG 6..10 a=2;\n"; // 5
		//Spec[] all_specs = Env.loadSpecString("SPEC (agent1 KNOW TRUE) ");
		Spec[] all_specs = Env.loadSpecString(to_parse);
		System.out.println("========= DONE Loading Specs ============");

		for (int i = 0; i < all_specs.length; i++) {
			System.out.println(i+" isRealTimeCTLSpec: " + all_specs[i].isRealTimeCTLSpec());

		CTLModelChecker checker;//与CTL*K一致
		try {
			checker = new CTLModelChecker(main, true);
		} catch (ModelCheckException e) {
			e.printStackTrace();
		...
		// model checking a module
		checker.modelCheckStandardOutput(all_specs[i]);
		}
	}
	...
	public static void rtctlkCheck() throws ModelCheckException {
		...
		String to_parse="";
		      ...
		      to_parse += "CTLSPEC ABF 6..10 (dc1 KNOW dc3.paid);\n"; // 2
		      to_parse += "CTLSPEC EBF 6..10 (dc1 KNOW dc2.said);\n"; // 3
		      to_parse += "CTLSPEC EBG 6..10 dc2.paid;\n"; // 4
		      to_parse += "CTLSPEC ABG 6..10 dc3.said;\n"; // 5
		      //to_parse += "SPEC (dc1 KNOW dc3.paid) & E [dc1.said BU 6..10 dc3.paid];\n";//VVV
		// to_parse += "SPEC v1 & !! A [ v1 U v2 ]\n";
		...
		Spec[] all_specs = Env.loadSpecString(to_parse);
		System.out.println("========= DONE Loading Specs ============");
		RTCTLKModelChecker checker = new RTCTLKModelChecker(main);
		// model checking a module
		System.out.println("CTLK"+!all_specs[0].isCTLKSpec()+" "+"RTCTL"+all_specs[0].isRealTimeCTLSpec()+" "+"RTCTLK"+!all_specs[0].isRealTimeCTLKSpec());
		checker.modelCheckStandardOutput(all_specs[0]);
		checker.modelCheckStandardOutput(all_specs[1]);
		checker.modelCheckStandardOutput(all_specs[2]);
		checker.modelCheckStandardOutput(all_specs[3]);
